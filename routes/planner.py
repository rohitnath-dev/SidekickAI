...
def load_cached_briefing(user_id: int) -> dict | None:
    ...
    if os.path.exists(path):
        try:
            ...
            logger.error("Failed to load cached briefing for user %d: %s", user_id, e)
    return None
...
def cache_briefing(user_id: int, data: dict) -> None:
    ...
    try:
        ...
        logger.error("Failed to cache briefing for user %d: %s", user_id, e)
...
async def daily_briefing(
    ...
    # Check if there are active integrations first, just like before
    ...
    if cached:
        logger.info("Serving daily briefing from cache for user %d", current_user.id)
    ...
    # 2. If no cache exists, return a fallback indicating no briefing has been generated yet.
    # Note that we do NOT call the LLM/PlannerAgent here to avoid automatic LLM execution!
    logger.info("No cached briefing found for user %d, returning fallback state", current_user.id)
    return BriefingResponse(
        date=today_date,
        executive_summary="No executive briefing has been generated yet. Please click 'Run AI' to analyze your data.",
        critical_items=[],
        pending_work=[],
        upcoming_deadlines=[],
        recommended_priorities=[],
        risks=[],
        next_actions=["Click 'Run AI' to generate a daily briefing."],
    )
